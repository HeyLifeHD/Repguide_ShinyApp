    ...
    server <- function(input, output) {
        ...
        blacklist_regions <- reactive({
            if (isFALSE(input$blacklist)) {
                ...
                promoter_ids <- mapIds(org(), keys=essentials$AAMP, keytype="SYMBOL", column="ENTREZID", multiVals="first")
                    # select(
                    #     org(),
            ...
            content = function(file) {
                ...
                for (i in kmers_by_nguides$n_guides)
                {
                    ...
                    Biostrings::writeXStringSet(
                        ...
                        filepath = paste0("Repguide_output/", i, '_guides_sequence.fasta'),
                        format = 'fasta'
                    )
                }
                ...
                {
                    fn <- paste0("Repguide_output/", 'combinations', '_', i, '.png')
                    ggsave(fn, p, device = 'png', dpi = dpi)
                    i <- i + 1
                }
